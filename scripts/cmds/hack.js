...
module.exports = {
  config: {
    ...
    shortDescription: {
      en: "Facebook hacking screenshot with the user's profile picture.",
    },
    guide: {
  ...
  hackMessage: async function ({ api, event, name }) {
    try {
      const loadingMessage = await api.sendMessage(
        `⏳ | Cracking Facebook Password for ${name}, Please wait...`,
        event.threadID
      );
      ...
      try {
        await api.editMessage(
          `⚠️ | Login failed! 2FA is enabled on *${name}*'s account.`,
          loadingMessageID
        );
      } catch (error) {
        await api.sendMessage(
  ...
  onStart: async function ({ args, usersData, threadsData, api, event }) {
    try {
      ...
      return api.sendMessage(
        {
          body: `✅ | Successfully hacked *${name}*'s account`,
